...
actor CafeteriaCanister {
  ...
  type Indice = Nat;
  var indice: Indice = 0;
  let usuario = HashMap.HashMap<Text, Usuario>(0, Text.equal, Text.hash);
...
public func actualizarUsuario (id:Text, nombre:Text, apellidop:Text, apellidom:Text, telefono:Text) : async Bool {
  ...
  public query func buscarProducto () : async [(Text, Producto)]{
  ...
  public query func buscarProductosid (id: Text) : async ?Producto {
    let product: ?Producto = Productos.get(id);
    return product;
  };
  ...
  public func listaProductos(id:Text):async Text {
    let product: ?product = Productos.get(id);
    if (product != null) {
